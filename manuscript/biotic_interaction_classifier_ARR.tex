% ACL Rolling Review format — official ACL Author Kit.
% \usepackage[review]{acl} for the anonymous review version (current);
% switch to \usepackage{acl} (no "review" option) for the camera-ready version.
\documentclass[11pt]{article}
\PassOptionsToPackage{hyphens}{url}  % let long URLs/model names break at hyphens, not just "/"
...
% Author block redacted in this anonymous-review mirror (the .tex source
% itself is publicly visible here, not just the compiled PDF).
\author{Anonymous}

\begin{document}
\maketitle
\nolinenumbers  % the lineno package misrenders under this document's two-column
                % float/spacing customizations; line numbers aren't an ARR requirement.
...
F1 is the best \emph{cold-start} result per scheme (full ablation in
Appendix~\ref{app:fullablation}). \texttt{full\_typed} lacks the highest
cold-start F1 but is carried to the warm-start ablation (0.874,
Table~\ref{tab:main_results}) for its HOST/PATHOGEN typing (Section~\ref{sec:zerocost}).}
...
\begin{description}
    ...
    gradient updates from token-level cross-entropy loss $\mathcal{L}_\text{NER}$,
    priming the encoder to locate taxon names and interaction verbs first.
...
encoder already encodes biotic interaction patterns from classification
fine-tuning, and NER pre-training updates it toward entity-detection
representations instead, degrading the classification signal.
Warm-starting without Phase 1 preserves this initialization while the
NER head still learns entity recognition jointly in Phase 2.
...
gen-set-150-extension (97 additional synthetic sentences restoring
the test set to 500). Source construction and validation details are
in Section~\ref{sec:contamination} and Appendix~\ref{app:contamination}.
