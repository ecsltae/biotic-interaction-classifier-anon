#!/usr/bin/env python3
"""
Build v14_unfiltered Dataset: Same as v14 but WITHOUT the score>0 filter

This is an experiment to test whether the score>0 filter on SIBiLS data
caused the v14 regression (v14 F1=0.706 vs v11_1 F1=0.781).

Hypothesis: The filter removes valid positives (especially dispersal, pollination,
symbiosis) which may actually help the model generalize better.

Changes from build_v14_dataset.py:
  - filter_positives=False for sibils_diverse and globi_sibils
  - Output to training_data_v14_unfiltered.csv
"""
import argparse
import sys
...
def main():
    ...
    parser.add_argument("--output", default=str(BASE_DIR / 'data/training/training_data_v14_unfiltered.csv'))
    parser.add_argument("--max-pos-per-type", type=int, default=500)
    args = parser.parse_args()

    print("=" * 70)
    print("BUILDING V14_UNFILTERED DATASET (no score>0 filter)")
    print("=" * 70)
    ...
    # ── 3. globi_sibils_real ──────────────────────────────────────────────────
    print("\n3. Adding globi_sibils_real (NO FILTER)...")
    ...
    print("\n" + "=" * 70)
    print("V14_UNFILTERED DATASET SUMMARY")
    print("=" * 70)
